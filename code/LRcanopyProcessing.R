...
canopy <-read.csv("C:\\Users\\Laura\\Documents\\ABRP\\Cleanup\\Canopy_Cleanup\\Canopy_for_R.csv", header=T, sep=",") #original code= pine <- read_csv("data/pine.csv")
...
INDCOVERYear <-groupedCanopy %>%
 ...
 ggplot(aes(x= MON_YEAR, y= meanIND, group = COVER, color = COVER)) +
  geom_line(size=1.5) +
  geom_point(size =  2.0) +
